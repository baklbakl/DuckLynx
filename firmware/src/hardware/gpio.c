...
// void setAFSEL(const uint32_t baseAddress, const uint8_t pins) {
...
void gpio_enableAnalog(const uint32_t baseAddress, const uint8_t pins) {
    *(volatile uint32_t * const)(baseAddress + REGISTER_GPIO_AFSEL_OFFSET) |= pins;
    *(volatile uint32_t * const)(baseAddress + REGISTER_GPIO_AMSEL_OFFSET) |= pins;
}

// void gpio_enableDigital(const uint32_t baseAddress, const uint8_t pins) {
