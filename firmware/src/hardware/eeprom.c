...
int8_t eeprom_init(void) {
    ...
    //If these errors happen it may be wise to reset and see if they persist
    ...
    *REGISTER_SYSCTL_RESET_EEPROM &= ~(REGISTER_SYSCTL_PERIPHCTL_EEPROM_INSTANCEMASK);
    ...
    //This is designed to bring up new MCUs as their programed address will be 0 or 255
    if(rhspAddress == 0 || rhspAddress == 0xFF) {
        eeprom_setAddress(1);
    }
...
//Don't use this unless it is before eeprom_setAddress could possibly be written.
...
void eeprom_setAddress(uint8_t newAddress) {
    ...
    *REGISTER_EEPROM_EEBLOCK &= ~REGISTER_EEPROM_EEBLOCK_MASK;
    *REGISTER_EEPROM_EEOFFSET &= ~REGISTER_EEPROM_EEOFFSET_MASK;
    *REGISTER_EEPROM_EERDWR = (uint32_t) newAddress;
}
